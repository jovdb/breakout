...
export class PaletteSystem {
	...
	private maxHorizontalBallVelocity: number;
	...
	public start(): () => void {
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			else if (e.which === 39) this.direction = "right";
		};

		const handleKeyUp = (e: KeyboardEvent) => {
			if (e.which === 37 && this.direction === "left") this.direction = "";
			if (e.which === 39 && this.direction === "right") this.direction = "";
		};
		...
		const broadcasterUnsubscribe = broadcaster.subscribeOnMessage("Collision", message => {
			...
			if (
				...
				if (message.entity.hasComponents(gravityComponent)) message.entity.removeComponents(gravityComponent);

				// Ball falls on top of palette?
				if (
					isPalette(message.collidedWith) &&
					message.side === "top"
				) {
	...
	public update(palette: IPalette) {
	...
	private entityToPalletCollision(palette: IPalette, entity: IEntity & IPosition & ISize & IVelocity) {
		...
		// Limit speed
		if (entity.dx > this.maxHorizontalBallVelocity) entity.dx = this.maxHorizontalBallVelocity;
		if (entity.dx < -this.maxHorizontalBallVelocity) entity.dx = -this.maxHorizontalBallVelocity;
	}
}
